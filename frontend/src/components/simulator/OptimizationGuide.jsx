...
const optimizationTopics = [
  ...
  {
    title: 'Optimizing ML/DL Systems',
    icon: <Zap className="h-5 w-5 mr-2" />,
    content: (
      <ul className="list-disc pl-5 space-y-2">
        <li>Use <strong>Model Caching</strong> to avoid redundant inference calls for the same inputs.</li>
        <li>Implement <strong>Batch Inference</strong> to maximize GPU utilization and reduce per-request latency.</li>
        <li>Deploy <strong>Model Quantization</strong> (INT8/FP16) to reduce model size, latency, and cost.</li>
        <li>Use <strong>Feature Stores</strong> to ensure consistency between training and serving features.</li>
        <li>Implement <strong>A/B Testing</strong> infrastructure for safe model deployment and validation.</li>
        <li>Monitor <strong>Model Drift</strong> and set up automated retraining pipelines.</li>
        <li>Use <strong>Edge Inference</strong> for latency-sensitive applications to reduce network overhead.</li>
        <li>Optimize <strong>GPU Memory</strong> usage with techniques like gradient checkpointing and mixed precision.</li>
      </ul>
    ),
  },
];
