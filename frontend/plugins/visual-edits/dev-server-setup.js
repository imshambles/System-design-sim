...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all system-design-sim.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*system-design-sim\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@system-design-sim.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@system-design-sim.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
